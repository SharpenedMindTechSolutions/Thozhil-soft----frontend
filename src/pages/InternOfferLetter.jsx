...
function InternOfferLetter() {
  ...
  const handleChange = (e) => {
    ...
    // Convert only intern_id to uppercase
    const newValue = name === "intern_id" ? value.toUpperCase() : value;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(data.message || "Intern details submitted successfully!", {
          position: "top-right",
          autoClose: 3000,
        ...
        toast.error(data.message || "Intern ID already exists!", {
          position: "top-right",
          autoClose: 3000,
          hideProgressBar: false,
          pauseOnHover: true,
      ...
      toast.error("Failed to submit intern details.", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        pauseOnHover: true,
  ...
  const backButtonStyle = {
    ...
    fontWeight: 500,
    fontSize: "14px",
    boxShadow: "0 3px 8px rgba(26,115,232,0.3)",
    transition: "all 0.3s",
  };
  ...
  return (
    <div style={containerStyle}>
      ...
      {/* Go Back Button */}
      <button style={backButtonStyle} onClick={() => navigate("/letters")}>
        <FaArrowLeft /> Go Back
      </button>
